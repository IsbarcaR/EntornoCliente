  ...
  while (modoActual === "SUPERVIVENCIA" || rondasSuperadas < maxRondas) {
    ...
    for (let i = 0; i < secuencia.length; i++) {
      let respuesta = prompt(
        "Ingresa el color número " + (i + 1) + " de la secuencia:"
      ).toLowerCase();
...
function guardarHistorial(resultado) {
  let tipoJuego =
    modoActual === "SUPERVIVENCIA" ? "MODO SUPERVIVENCIA" : "MODO CLASICO";
  let registro = `[${partidasJugadas + 1}] ${tipoJuego}: ${
    colores.length
  } colores. ${resultado}.`;
...
function cambiarModo() {
  let nuevoModo = prompt(
    "Elige un modo de juego: CLASICO o SUPERVIVENCIA"
  ).toUpperCase();
...
function cambiarColores() {
  let nuevosColores = prompt(
    "Ingresa los colores separados por comas (mínimo 4):"
  )
    .split(",")
